...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    await redis.hset(`user:${userId}:config`, 'ENABLE_TRADING', 'false');

    res.status(200).json({ message: 'Trading stopped successfully' });
  } catch (error) {
    // Log the error instead of terminating the process
    console.error('Error stopping trading:', error);
    res.status(500).json({ error: 'Failed to stop trading. Please check the server logs for more details.' });
  }
}
